...
for(let i=0; i<artists.length; i++){
  if(artists[i].name.includes('Vincent')) {
...
console.log(addArtist(artists, 'Jazmine Kime', '2001-now', 'Full Stack Web Development', 'American', 'The example sentence is too accurate.'))
...
function artistInfo(arr, nameIn){
  for(let i=0; i<arr.length; i++){
    if(arr[i].name === nameIn) {
      return arr[i].bio
    }
  }
  return;
...
function artistByCountry(arr, nation){
  const sameNat = [];
  for(let i=0; i<arr.length; i++){
      if(arr[i].nationality.toLowerCase() === nation.toLowerCase()) {
        sameNat.push(arr[i].name);
      }
  }
  return sameNat;
}
